...
export function DocumentList({ documents }: DocumentListProps) {
  ...
  const podDocs = documents.filter(
    (d) => d.document_type === ShipItemDocumentTypeEnum.PROOF_OF_DELIVERY,
  );
  const podDocumentDocs = documents.filter(
    (d) =>
      d.document_type ===
  ...
  const renderDocItem = (doc: ShipItemDocument) => (
    <div
      key={doc.id}
      className="flex items-center justify-between p-3 border rounded-md bg-card mb-2"
    >
      <div className="flex items-center gap-3 overflow-hidden">
        ...
        <div className="min-w-0">
          <p className="text-sm font-medium truncate">
            {doc.file_name ||
              `${doc.document_type.replace(/_/g, " ")}.${doc.file_ext}`}
          </p>
