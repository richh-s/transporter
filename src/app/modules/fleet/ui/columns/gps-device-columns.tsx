...
import { ArrowUpDown, MoreHorizontal, Edit, Power, Eye, Satellite, Truck, Clock } from "lucide-react";
...
export const gpsDeviceColumns: ColumnDef<GPSDeviceTableRow>[] = [
            ...
            return (
                <div className="flex flex-col min-w-[150px]">
                    <div className="flex items-center gap-2">
                        ...
                        <span className="font-bold text-sm">{device.external_device_id}</span>
                    </div>
    ...
    },
    {
        ...
        header: () => <span className="text-xs font-semibold uppercase tracking-wider">Device Info</span>,
        cell: ({ row }) => {
            const device = row.original;
            return (
                <div className="flex flex-col min-w-[120px]">
                    <span className="text-xs font-medium">{device.device_name || "N/A"}</span>
                    <span className="text-[10px] text-muted-foreground">{device.device_model || "Common Model"}</span>
                </div>
            );
        },
    },
    {
        ...
        header: () => <span className="text-xs font-semibold uppercase tracking-wider">Expiration</span>,
        cell: ({ row }) => {
    ...
    {
        id: "assignment",
        header: () => <span className="text-xs font-semibold uppercase tracking-wider">Assignment</span>,
        cell: ({ row, table }) => {
            const device = row.original;
            ...
            return (
                <div className="flex items-center gap-1.5 text-xs min-w-[100px]">
                    <Truck className={cn("h-3.5 w-3.5", isAssigned ? "text-emerald-500" : "text-muted-foreground")} />
                    <span className={isAssigned ? "text-emerald-600 font-medium" : "text-muted-foreground"}>
            ...
            return (
                ...
                >
                    <Switch
                        ...
                        onCheckedChange={(checked) => meta.onStatusChange?.(device, checked)}
