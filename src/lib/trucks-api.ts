      ...
      try {
        ...
        if (error) {
          ...
          if (process.env.NODE_ENV === "development") {
            console.log(
              `[Trucks API] Endpoint ${endpoint} returned error:`,
              error,
              `(Status: ${status})`,
            );
          }
        ...
        if (data) {
          ...
          if (Array.isArray(data)) {
            if (process.env.NODE_ENV === "development") {
              console.log(
                `[Trucks API] Successfully fetched ${data.length} trucks from ${endpoint}`,
              );
            }
            return data;
          }
          ...
          if (
            typeof data === "object" &&
            "items" in data &&
            Array.isArray((data as { items: Truck[] }).items)
          ) {
            const items = (data as { items: Truck[] }).items;
            if (process.env.NODE_ENV === "development") {
              console.log(
                `[Trucks API] Successfully fetched ${items.length} trucks from ${endpoint}`,
              );
            }
            return items;
          }
          ...
          if (
            typeof data === "object" &&
            "data" in data &&
            Array.isArray((data as { data: Truck[] }).data)
          ) {
          ...
          if (
            typeof data === "object" &&
            "trucks" in data &&
            Array.isArray((data as { trucks: Truck[] }).trucks)
          ) {
          ...
          if (
            typeof data === "object" &&
            "result" in data &&
            Array.isArray((data as { result: Truck[] }).result)
          ) {
          ...
          if (process.env.NODE_ENV === "development") {
            console.log(
              `[Trucks API] Endpoint ${endpoint} returned data but format not recognized:`,
              data,
            );
          }
        }
        ...
        if (process.env.NODE_ENV === "development") {
          console.log(
            `[Trucks API] Endpoint ${endpoint} failed:`,
            endpointError,
          );
        }
        continue;
